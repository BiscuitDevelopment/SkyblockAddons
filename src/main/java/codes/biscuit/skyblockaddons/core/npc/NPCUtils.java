...
public class NPCUtils {
    ...
    public static boolean isMerchant(String NPCName) {
        for (NPC npc:
             NPC_LIST) {
            if (NPCName.equals(npc.getName()) && npc.isType(NPCType.MERCHANT)) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isFullMerchant(String NPCName) {
        for (NPC npc:
                NPC_LIST) {
            if (NPCName.equals(npc.getName()) && npc.isType(NPCType.MERCHANT) && !npc.isType(NPCType.BUY_ONLY)) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Checks if a given entity is near any NPC with the given {@link NPCType}.
     *
     * @param entity the entity to check
     * @return {@code true} if the entity is near an NPC, {@code false} otherwise
     */
    public static boolean isNearAnyNPCWithType(Entity entity, NPCType type) {
        SkyblockAddons main = SkyblockAddons.getInstance();
        Location currentLocation = main.getUtils().getLocation();
    ...
    /**
     * Checks if a given entity is near any NPC with the given {@link NPCType}s.
     *
     * @param entity the entity to check
     * @return {@code true} if the entity is near an NPC with the given types, {@code false} otherwise
     */
    public static boolean isNearAnyNPCWithTypes(Entity entity, Set<NPCType> types) {
        SkyblockAddons main = SkyblockAddons.getInstance();
        Location currentLocation = main.getUtils().getLocation();
        ...
        for (NPC npc:
            ...
            if (npc.hasLocation(currentLocation)) {
                ...
                if (NPCVector.distanceTo(entityVector) <= HIDE_RADIUS) {
                    if (npc.getTypes().containsAll(types))
                        return true;
                }
            }
        }
    ...
    public static boolean isNearAnyFurnitureNPC(Entity entity) {
        ...
        for (NPC npc:
            ...
            if (npc.hasLocation(currentLocation)) {
                ...
                if (NPCVector.distanceTo(entityVector) <= HIDE_RADIUS) {
                    if (npc.isType(NPCType.FURNITURE))
                        return true;
                }
            }
        }
