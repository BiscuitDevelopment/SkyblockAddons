...
public class GuiContainerTransformer implements ITransformer {
    ...
    private InsnList onHandleMouseClick() {
        ...
        list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "codes/biscuit/skyblockaddons/asm/hooks/GuiContainerHook", "onHandleMouseClick",
                "("+TransformerClass.Slot.getName()+"III)Z", false));
        LabelNode notCancelled = new LabelNode(); // if (GuiContainerHook.onHandleMouseClick(slotIn, slotId, clickedButton, clickType))
        list.add(new JumpInsnNode(Opcodes.IFEQ, notCancelled));
